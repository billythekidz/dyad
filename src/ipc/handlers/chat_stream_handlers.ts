...
- Long-running commands (servers) should use \`&\` to run in background`,
    ...
    execute: async (args: any) => {
      ...
      try {
        logger.log(`[Bash Tool] Executing: ${command}`);
        logger.log(`[Bash Tool] Description: ${description || 'N/A'}`);
        ...
        const output = [stdout, stderr].filter(Boolean).join('\n\n');
        logger.log(`[Bash Tool] Success! Output:\n${output.slice(0, 500)}...`);
        ...
        logger.error(`[Bash Tool] Error executing command:`, errorMessage);
